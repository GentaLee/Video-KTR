...
原核验日期：2026-09-24 UTC。**KTR训练完成并保存成功。**

2026-09-25补记：baseline也已完成；本次重算KTR四个权重哈希与下文一致，未被覆盖。见[配对报告](../b200-paired-20260925/COMPARISON.md)。独立质量评测尚未运行。
...
baseline已从相同SFT完成全量2115步，未加载KTR状态，数据与采样器一致。下一步在同一独立评测集评测两个最终模型，再比较质量、资源和token示例。
